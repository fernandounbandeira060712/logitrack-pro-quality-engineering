...
class ApplicationMappingTests extends BaseTest {
    ...
    @Test
    @DisplayName("DISCOVERY-001 - Mapear links da Ã¡rea autenticada")
    void shouldMapAuthenticatedNavigationLinks() {
        ...
        List<String> links =
                dashboard.sidebar().getVisibleNavigationItems();

        System.out.println("URL autenticada: " + page.url());
        System.out.println("TÃ­tulo: " + page.title());
        System.out.println("Menu identificado: " + links);

        assertFalse(
                links.isEmpty(),
                "Nenhum item de navegaÃ§Ã£o foi identificado."
        );
    }
}